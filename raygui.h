...
#define RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY      1        // Frames delay for autocursor movement
        ...
        (wrapMode == TEXT_WRAP_NONE))               // No wrap mode
    {
        ...
        if (editMode)
        {
            ...
            {
                int enterCodepoint = (int)'\n';
                int enterSize = 0;
                const char* enterEncoded = CodepointToUTF8(enterCodepoint, &enterSize);

                if ((textLength + enterSize) < textSize)
                {
                    for (int i = (textLength + enterSize); i > textBoxCursorIndex; i--) text[i] = text[i - enterSize];
                    for (int i = 0; i < enterSize; i++) text[textBoxCursorIndex + i] = enterEncoded[i];
                    textBoxCursorIndex += enterSize;
                    textLength += enterSize;
                    text[textLength] = '\0';
                }
            }

            // 3. 【核心修改】使用 while 循环一次性读取所有输入字符
            ...
            while (codepoint > 0)
            {
                ...
                // 判断字符有效性 (注意：这里允许所有 >= 32 的字符，支持中文)
                if ((codepoint >= 32) && ((textLength + codepointSize) < textSize))
                {
                    // 移动光标后的数据
